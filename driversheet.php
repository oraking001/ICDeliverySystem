...
<script type="text/javascript">
 ...
 <!-- BEGIN CONTENT -->
            ...
            <input type="text" class="form-control" id="myInput" onkeyup="myFunction()" placeholder="Search for Driver Sheet.." title="Type in a Driver Sheet">        
            </div>
                        <table class="table table-checkable table-bordered table-hover table-responsive" id="transit_table">

                            <thead>
								<tr>
									<th style="width:15%;">Driver Sheet</th>
									<th>Origin Hub</th>
  ...
  <div class="modal-dialog" style="background:white;">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <!-- END CONTAINER -->
        <form method="POST" id="print_form" action="dm_api/Driversheet/print_data" target="_blank">
                  <input type="hidden" id="driversheet" name="driversheet" value="">
                </form>
...
<script type="text/javascript">
 ...
 $(document).ready(function (){
    ...
    function datashow(){
         $.ajax({
             type: "POST",
             url: "dm_api/Driversheet/get_driversheet_data",
             data: {
               ...
               // $("#transit_table").dataTable().fnDestroy();
                 ...
                 for (var i = 0; i < data.length; i++) {
                   ...
                   if(data[i].status == 4){
                     ...
                     '<td><a id="'+data[i].driver_sheet+'" class="receipt">'+ data[i].driver_sheet + '</a></td>' +
                     '<td>' + data[i].hub_name + '</td>' +
                     '<td>' + data[i].destination_hub + '</td>' +
                     '<td>' + data[i].tracking_details.length + '</td>' +
                     '<td>' + data[i].Created_at + '</td>' +
                     '<td>' + data[i].sau_name + '</td>' +
                     '<td>' + data[i].delivery_at + '</td>' +
                     '<td>' + data[i].driver_name + '</td>' +
                     '<td id="status_' + data[i].id + '"><span class="'+cls+'" style="color:#000000;">' + status + '</span></td>' +
                     '<td><button type="button" name="edit" class="print_data  btn btn-xs btn-success" id="' + data[i].driver_sheet + '"><i class="fa fa-print"></i></button></td>' +
                     '</tr>' +
        ...
        $(document).on("click", ".print_data", function(e) {
            e.preventDefault();
            var driversheet = $(this).attr('id');
            $('#driversheet').val(driversheet);
            $('#print_form').trigger('submit');
        });
