$(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function(data) {console.log("Result:", data);
                ...
                for (var i = 0; i < data.length; i++) {
                    if(table == 'tbl_hub'){
                        html += '<option value="'+data[i].id+'">'+data[i].hub_name+'</option>';
                    }else{
                        html += '<option value="'+data[i].sa_id+'">'+data[i].sau_FName+'</option>';
                    }
    ...
    var generateRandomNDigits = (n) => {
     ...
     function datashow(hub_id){
         $.ajax({
             type: "POST",
             url: "dm_api/Driversheet/get_data",
             data: {
           ...
           if(cid == ''){
            ...
            function() {
                $.ajax({
                    data: {
                        tracking_no: cid,
                        driver_id: destination_hub,
                        driver_sheet: transit_bag,
                    },
                    url: "dm_api/Driversheet/update_data",
                    type: "POST",
                    dataType: 'json',
                    // async: false,
